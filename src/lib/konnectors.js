...
function sanitizeFuckingSlugWorthlessPieceOfShit (konnector) {
...
export function findBySlug (cozy, slug) {
  ...
  return getSlugIndex(cozy)
    ...
    .then(konnector => sanitizeFuckingSlugWorthlessPieceOfShit(konnector))
}
...
export function findAll (cozy) {
  return findAllDocuments(cozy, KONNECTORS_DOCTYPE)
    .then(konnectors => konnectors.map(sanitizeFuckingSlugWorthlessPieceOfShit))
}
...
export function install (cozy, konnector, timeout = 120000) {
  ...
  return findBySlug(cozy, slug)
    ...
    .then(sanitizeFuckingSlugWorthlessPieceOfShit)
}
